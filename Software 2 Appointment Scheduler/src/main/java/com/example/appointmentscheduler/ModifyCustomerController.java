...
public class ModifyCustomerController implements Initializable {
    ...
    private static Customer modifymeCustomer;
    public static void addToModify(Customer customer){
        modifymeCustomer = customer;
    }
    ...
    public void modifyCustomerButton(ActionEvent actionEvent) throws IOException, SQLException {
        ...
        alert.setContentText("Are you sure you want to modify the customer?");

        if (alert.showAndWait().get()== ButtonType.OK) {
            try {
                id = modifymeCustomer.getCustomerId();
            ...
            if(DatabaseCustomerQuery.update(id,n,a,post,c,div,phone) == 1){
                toMain(actionEvent);
            }
            else{
                Alert unableto = new Alert(Alert.AlertType.ERROR);
                unableto.setHeaderText("Unable to modify Customer");
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        customerID.setText(Integer.toString(modifymeCustomer.getCustomerId()));
        addPostal.setText(modifymeCustomer.getPostalCode());
        addName.setText(modifymeCustomer.getCustomerName());
        addAddress.setText(modifymeCustomer.getAddress());
        addPhone.setText(modifymeCustomer.getPhoneNumber());
        try {
            addCountry.setValue(DatabaseCustomerQuery.getCountryFromDivID(modifymeCustomer.getDivisionID()));
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
        try {
            division.setValue(DatabaseCustomerQuery.getDivNameFromID(modifymeCustomer.getDivisionID()));
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
